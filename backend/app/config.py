...
class Settings:
    ...
    chat_url_override: str = os.getenv("CHAT_URL", "").rstrip("/")
    ...
    embeddings_url_override: str = os.getenv("EMBEDDINGS_URL", "").rstrip("/")
    rerank_model: str = os.getenv("RERANK_MODEL", "qwen3-rerank")
    rerank_base_url: str = os.getenv(
        "RERANK_BASE_URL", "https://dashscope.aliyuncs.com/compatible-api/v1"
    ).rstrip("/")
    rerank_url_override: str = os.getenv("RERANK_URL", "").rstrip("/")
    ...
    def chat_url(self) -> str:
        if self.chat_url_override:
            return self.chat_url_override
        return f"{self.llm_base_url}/chat/completions"

    @property
    def embeddings_url(self) -> str:
        if self.embeddings_url_override:
            return self.embeddings_url_override
        base = self.embedding_base_url or self.llm_base_url
        return f"{base}/embeddings"

    @property
    def rerank_url(self) -> str:
        if self.rerank_url_override:
            return self.rerank_url_override
