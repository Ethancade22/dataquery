...
class DuckDbEngine:
    ...
    def clean_sql(text: str) -> str:
        ...
        cleaned = DuckDbEngine._normalize_dateadd(cleaned)
        return cleaned.strip()

    @staticmethod
    def _normalize_dateadd(sql: str) -> str:
        """Convert common DATEADD output from LLMs into DuckDB interval arithmetic."""

        pattern = re.compile(
            r"\bDATEADD\s*\(\s*'?([A-Za-z]+)'?\s*,\s*([+-]?\d+)\s*,\s*"
            r"([A-Za-z_][\w.]*|CURRENT_DATE|CURRENT_TIMESTAMP)\s*\)",
            flags=re.IGNORECASE,
        )
        ...
        def replace(match: re.Match[str]) -> str:
            ...
            return f"({expression} {operator} INTERVAL '{abs(amount)} {unit}')"
