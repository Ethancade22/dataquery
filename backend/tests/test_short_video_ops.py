...
class ShortVideoOpsDataTest(unittest.TestCase):
    ...
    def test_duckdb_engine_normalizes_common_dateadd_output(self) -> None:
        engine = DuckDbEngine()

        execution = engine.execute(
            DATABASE,
            "SELECT COUNT(ad_stat_id) AS stat_count FROM ad_daily_stats "
            "WHERE stat_date < DATEADD('month', 1, CURRENT_DATE)",
        )

        self.assertTrue(execution.success, execution.error)
        self.assertIn("INTERVAL '1 month'", execution.sql)
